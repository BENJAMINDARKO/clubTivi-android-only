...
class SeriesScreen extends ConsumerStatefulWidget {
  ...
  const SeriesScreen({super.key, this.category, this.providerId});
...
class _SeriesScreenState extends ConsumerState<SeriesScreen> {
  static const _streamType = 'series';
  ...
  Future<void> _buildCategoryList() async {
    _allCategories = ['All Series', 'Recently Added'];
  ...
  void _onChannelFocus(db.Channel ch) async {
    ...
    final meta = await scraper.fetchSeriesMetadata(ch.name);
    CinemetaShowDetail? detail;
    if (meta != null) {
        detail = await scraper.getTvShow(meta.id);
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (_providers.isEmpty || _allCategories.isEmpty) {
      return Scaffold(
        ...
        appBar: AppBar(backgroundColor: Colors.black, title: const Text('Series')),
        body: Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: const [
              Icon(Icons.tv_rounded, size: 64, color: Colors.white24),
              SizedBox(height: 16),
              Text('No series content found',
